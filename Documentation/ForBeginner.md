...
- **前端**：Vue 3 + Vite + TypeScript，负责界面和交互（位于仓库根目录）。
- **后端**：Express.js + TypeScript + SQLite，负责 API、鉴权和数据（位于 `server/`）。
...
3. **配置后端环境变量**
   ...
   - 默认数据库位置为 `server/data/library.db`，初学者无需改动。
...
7. **验证一切正常**
   - 浏览器访问前端地址，尝试登录/浏览图书。
   - 如需查看 API 列表，进入 `server/API_DOCUMENTATION.md` 或访问 Swagger（若已暴露）。
...
| 根目录 | `npm run build` | 构建前端产物（`dist/`） |
| 根目录 | `npm run type-check` | 前端 TypeScript 检查 |
...
- 📄 **API 文档**：`server/API_DOCUMENTATION.md`（涵盖认证、图书、借阅等所有端点）  
...
1. **理解请求流程**  
   ...
   - 对照 `server/src/routes` 和 `controllers` 查看后端如何处理请求。

2. **阅读数据模型**  
   - `server/src/models` 展示了 SQLite 查询语句和业务字段，适合理解数据库结构。
...
4. **运行测试并调试失败用例**  
   - 分析 `server/src/scripts/apiTest.ts`，了解如何使用 axios 进行端到端测试。
...
2. 有问题可：
   - 查看 `server/README.md` 与 `server/API_DOCUMENTATION.md` 获取更详细的后端说明；
