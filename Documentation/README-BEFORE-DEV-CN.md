...
## 14. 收藏与信用功能要点（新增）
- 收藏 API：`GET/POST /api/users/:id/favorites`、`DELETE /api/users/:id/favorites/:bookId`，仅本人或管理员/图书管理员访问。
- 信用 API：`GET /api/users/:id/credit`（本人/管理员/图书管理员），`PUT /api/users/:id/credit` 仅管理员/图书管理员；只允许修改 `score`（0-100）与 `remarks`，等级/状态由分数自动推导。
- 信用恢复：每满 3 天自动 +10 分，封顶 100（访问信用接口时触发）；默认分 80。
- 业务限制：分数 <50 禁借阅/评分/评论；分数 <70 禁续借；后端已强制借阅/续借限制，前端可按需要同步禁用按钮。
